"""
Email Service for notifications using Resend API
"""
...
def send_email_via_resend(to_email, subject, html_content, attachments=None):
    ...
    try:
        ...
        # Iterate to ensure 'EventHub' name is used if sender is default
        if sender_email == 'onboarding@resend.dev':
             from_field = "EventHub <onboarding@resend.dev>"
        else:
...
def send_ticket_confirmation(user_email, user_name, event_title, ticket_number, quantity, total_price):
    ...
    """
    return send_email_via_resend(user_email, f'Ticket Confirmed - {event_title}', html)
...
def send_event_reminder(user_email, user_name, event_title, event_date, event_location):
    ...
    <p>See you there!</p>
    """
    return send_email_via_resend(user_email, f'Reminder: {event_title} is coming up!', html)
